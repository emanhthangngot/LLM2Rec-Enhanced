...
class PreflightTests(unittest.TestCase):
    def test_coverage_and_manifest_gate(self) -> None:
    ...
    def test_coverage_gate_rejects_shortfall(self) -> None:
        report = make_coverage_report(range(10), range(7), range(5), range(2))
        with self.assertRaises(RuntimeError):
            validate_coverage(report)

    def test_manifest_requires_immutable_clip_revision(self) -> None:
            ...
            validate_manifest(
                {
                    ...
                    "catalog_count": 1,
                    "test_target_count": 1,
                    "coverage": make_coverage_report([1], [1], [1]).to_dict(),
                }
            )
