...
# --------------------------------------------------------------------------
# Orchestration: SFT-only push (script kernel -- no notebook/nbclient cell
# involved, avoiding the confirmed 1800s Papermill cell-reply timeout that
# cancelled a prior long-running push in this same repo; see
# plans/260915-0955-visual-delta-fusion-pilot/ISSUES.md#22). Kaggle's own
# platform execution cap is a real, separately-confirmed ~21,600s (6h); this
# push's own soft budget below is set well under that.
# --------------------------------------------------------------------------
...
EXPERIMENT_CONFIG["budget_seconds"] = 4500.0

INPUT_ROOT = _Path("/kaggle/input")
OUTPUT_ROOT = _Path("/kaggle/working/hanorec_cf_hardness_output")
OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

print({"stage": "prepare_start", "elapsed": _time.monotonic() - _KERNEL_START})
prepared = prepare(EXPERIMENT_CONFIG, INPUT_ROOT, OUTPUT_ROOT, _Path("/kaggle/working"))
print({"train_pairs": len(prepared["train"]), "eval_rows": len(prepared["evaluation"]), "catalog_items": len(prepared["catalog"]), "elapsed": _time.monotonic() - _KERNEL_START})
(OUTPUT_ROOT / "prepared_manifest.json").write_text(
    __import__("json").dumps({k: v for k, v in prepared.items() if k != "catalog"}, indent=2)
)

print({"stage": "run_sft_start", "elapsed": _time.monotonic() - _KERNEL_START})
sft_bundle = run_sft(EXPERIMENT_CONFIG, prepared, OUTPUT_ROOT, _Path("/kaggle/working"))
print({
    "stage": "run_sft_complete",
    "elapsed": _time.monotonic() - _KERNEL_START,
    "sft_losses": sft_bundle["sft_losses"],
    "reference_reproducibility_check": sft_bundle["reference_reproducibility_check"],
})
