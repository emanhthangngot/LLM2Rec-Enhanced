...
| **v10** *(active)* | `Caption-Augmentation` | Florence‑2 offline image→text captions injected into CSFT/MNTP/SimCSE input history via 5 controlled arms (`title-only`/`null`/`real`/`shuffle`/`paraphrase`) | **Running — `real` arm retrained after a history-budget fix; fix confirmed not the cause of the null result, run-to-run training variance now suspected instead; 4/5 arms remaining** | Games (dev), Arts (replication), AmazonMix-6 (pretrain); Baby reserved sealed | SASRec (matched) |
...
| `v10` `real` arm (v8, pre-fix, history-budget confound present) | SASRec | pre-fix retrain, ckpt-1000 | all rows | 0.0504 (`v0`) | 0.04866 | `-3.41%` | 0.0821 (`v0`) | 0.08253 | `+0.57%` | **Superseded** — see `v9` row and note below |
| `v10` `real` arm (v9, history-budget fix applied, full retrain) | SASRec | post-fix retrain, ckpt-1000 | all rows | 0.0504 (`v0`) | 0.04155 | `-17.56%` | 0.0821 (`v0`) | 0.07083 | `-13.73%` | **Preliminary, hypothesis retracted** — see note below |
...
**`v10` retraction, stated plainly:** a prior version of this README claimed a "common-history-token-budget" bug (captions inflating token count, causing tail truncation to silently drop more history items in the `real` arm than a title-only run) explained why `real ≈ v0`. That bug was real and was fixed in `code/.../full-pipeline/csft/runner.py` — but the fix's own instrumentation shows it changed **exactly 1 history item out of ~4.03 million** in the actual training corpus (`history_items_dropped_for_budget: 1`, `caption_csft_artifact.json`). The v8→v9 retrain (near-identical input data, same code otherwise) moved NDCG@10 from `0.04866` to `0.04155` — a `14.6%` swing from a change that touched one row. That swing is far larger than what a 1-item data difference could plausibly cause. Verified by source-code review (`external/.../llm2rec/run_csft.py`): this runner's compatibility patch actually **removes** the upstream `hf_train_dataset.shuffle(seed=seed)` call entirely and feeds data in its original file order — so dataset order was already identical across both runs, ruling out shuffle-seed drift as the explanation. No call to `set_seed()`/`torch.manual_seed()` exists anywhere in the training script (confirmed by direct grep, zero matches), so three other stochastic sources stay unseeded every run: HuggingFace `Trainer`'s own internal batch sampler, dropout during the forward/backward pass, and `fp16` mixed-precision reduction order on GPU (the compatibility patch substitutes `fp16` for the paper's `bf16`, a format with known run-to-run numerical drift). Compounded over `1,000` of the paper's `10,000` CSFT steps — an under-converged regime where the loss landscape is far from a stable minimum — this is sufficient to produce the observed swing. Neither `v8` nor `v9`'s single run is trustworthy evidence about caption effectiveness on its own; multiple seeds are required to separate a real caption effect from this now-confirmed retrain variance, in addition to the still-missing `title-only`/`null`/`shuffle`/`paraphrase` controls. `v11`'s three rows use `shuffle` (image-item link broken) as the baseline, matching the same control logic as `v9.2`'s `C0.5`; `real` beats `shuffle` at all three weights, on the full pre-registered 265-user population (verified: `candidate_recall@20 = 0.117` on every arm, matching SASRec's independently-known ~11% Recall@20 — confirms the sample is large enough to reflect the real retriever, unlike the 25-user pilot below). This is **one seed only**, not yet the family-wise significance test `experiment.json` calls for (3 seeds, hierarchical bootstrap) — real evidence, but not yet the registered confirmatory result.
